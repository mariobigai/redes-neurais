...
y = np.array([mapeamento[label] for label in y])
...
# Criar o modelo
model = Sequential()
model.add(Dense(4, input_dim=4, kernel_initializer='normal', activation='tanh')) #5 neuronios na primeira camada e 3 variaveis de entrada
...
plt.savefig(f'MSE por Épocas - Iris', dpi=500, orientation='portrait')
plt.show()
...
plt.savefig(f'Acurácia por Épocas - Iris', dpi=500, orientation='portrait')
plt.show()
...
plt.savefig(f'Matriz de confusão Iris', dpi=500, orientation='portrait')
plt.show()